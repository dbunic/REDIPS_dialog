...
Sep 06, 2011.
*/
"use strict";var REDIPS=REDIPS||{};REDIPS.dialog=(function(){var init,show,hide,image_tag,iframe_tag,position,visibility,fade,input_html,op_high=60,op_low=0,fade_speed=10,close_button='✕',youtube='<object width="640" height="390">'+'<param name="movie" value="http://_youtube_?&version=2&fs=0&rel=0&iv_load_policy=3&color2=0x6A93D4"></param>'+'<param name="allowFullScreen" value="true"></param>'+'<param name="allowScriptAccess" value="always"></param>'+'<embed src="http://_youtube_?&version=2&fs=0&rel=0&iv_load_policy=3&color2=0x6A93D4" '+'type="application/x-shockwave-flash" '+'allowfullscreen="true" '+'allowscriptaccess="always" '+'width="640" height="390">'+'</embed>'+'</object>',shade,dialog_box,dialog_width=0,dialog_height=0,function_call,function_param,dialog_id='REDIPS_dialog';init=function(){dialog_box=document.createElement('div');dialog_box.setAttribute('id',dialog_id);shade=document.createElement('div');shade.setAttribute('id','REDIPS_shade');var body=document.getElementsByTagName('body')[0];body.appendChild(shade);body.appendChild(dialog_box);REDIPS.event.add(window,'resize',position);REDIPS.event.add(window,'scroll',position);};show=function(width,height,text,button1,button2){var input1='',input2='',param='',img_extensions,page_extensions,youtube_url,youtube_html='',div_img='',div_text='',img_text='';dialog_width=width;dialog_height=height;position();img_extensions=/(\.jpg|\.jpeg|\.gif|\.png)$/i;page_extensions=/(\.php|\.html)/i;youtube_url=/www\.youtube\.com/i;if(img_extensions.test(text)){img_text=text.split('|');if(img_text.length===1){div_img=image_tag(img_text[0]);}
else{div_img=image_tag(img_text[1]);div_text='<div>'+img_text[0]+'</div>';}}
else if(page_extensions.test(text)){div_img=iframe_tag(text);}
...
dialog_box.innerHTML='<div class="REDIPS_titlebar"><span title="Close" onclick="REDIPS.dialog.hide(\'undefined\')">'+REDIPS.dialog.close_button+'</span></div>'+'<table class="REDIPS_table" cellpadding="0" cellspacing="0"><tr><td valign="center" height="'+height+'" width="'+width+'">'+
div_img+
div_text+
youtube_html+'<div>'+input1+input2+'</div>'+'</td></tr></table>';shade.style.display=dialog_box.style.display='block';visibility('hidden');fade(REDIPS.dialog.op_low,5);};hide=function(fnc,param){function_call=fnc;function_param=param;fade(REDIPS.dialog.op_high,-10);dialog_box.style.display='none';visibility('visible');};input_html=function(button){var param,html;button=button.split('|');param=button[2];if(param!==undefined){param='\',\''+param;}
else{param='';}
...
img+='</tr></table></div>';}
return img;};iframe_tag=function(url){var iframe;iframe='<iframe src="'+url+'" width="100%" height="100%" frameborder="0"></iframe>';return iframe;};position=function(){var window_width,window_height,scrollX,scrollY;if(typeof(window.innerWidth)==='number'){window_width=window.innerWidth;window_height=window.innerHeight;scrollX=window.pageXOffset;scrollY=window.pageYOffset;}
